...
function transformCampaignPost(
  p: CampaignPost,
  ...
  return {
    ...
    excerpt,
    content,
    image: resolveUrl(p.thumbnail) || tiptapFirstImage(p.content, resolveUrl) || PLACEHOLDER_IMAGE,
 ...
 * There is no local fallback content — an unreachable or empty API yields an
 * empty list and callers render their own empty state.
 */
export function useBlog() {
  ...
  const { data: postsData, pending } = useAsyncData(
    ...
    async () => {
      try {
        ...
        console.warn('Campaign posts API unavailable', e)
  ...
  const posts = computed<BlogPost[]>(() =>
    (postsData.value ?? []).map(p =>
      transformCampaignPost(p, resolveMediaUrl, site.value.name),
    ),
  )
  ...
  const getBySlug = async (slug: string): Promise<BlogPost | null> => {
    try {
      ...
      if (res.campaign_post) {
        return transformCampaignPost(res.campaign_post, resolveMediaUrl, site.value.name)
      }
    } catch (e) {
      // True 404 → fall through to the already-listed posts. Transport/SSR
      // failures rethrow so the page can show an error instead of a false
      // "not found".
...
export function useBlogTopics() {
  ...
  const getPostsByTopicSlug = async (slug: string): Promise<BlogPost[]> => {
    try {
      ...
      return (res.campaign_posts ?? []).map(p =>
        transformCampaignPost(p, resolveMediaUrl, site.value.name),
      )
    } catch (e) {
