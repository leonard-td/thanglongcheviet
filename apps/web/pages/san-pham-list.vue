<script setup lang="ts">
const { t } = useI18n()
const localePath = useLocalePath()

useScrollAnimation()

useSeoMeta({
  title: () => t('products.title'),
  description: () => t('products.subtitle'),
  ogTitle: () => t('products.title'),
  ogDescription: () => t('products.subtitle'),
  ogType: 'website',
  twitterCard: 'summary_large_image',
  twitterTitle: () => t('products.title'),
  twitterDescription: () => t('products.subtitle'),
})
</script>

<template>
  <div class="bg-dark text-white">
    <!-- <LayoutPageHero
      :label="t('products.label')"
      :title="t('products.title')"
      :subtitle="t('products.subtitle')"
    /> -->

    <SectionsProductCatalog />

    <SectionsCollectionShowcase />

    <SectionsBlogList
      :eyebrow="t('products.storyEyebrow')"
      :title="t('products.storyTitle')"
      :subtitle="t('products.storySubtitle')"
    />

    <section class="bg-primary-500 text-dark" aria-label="cta">
      <div class="container-page py-8 flex flex-col sm:flex-row items-center justify-between gap-4 text-center sm:text-left">
        <p class="font-heading text-lg md:text-xl font-semibold flex items-center gap-2">
          <svg class="w-5 h-5 flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />
          </svg>
          {{ t('products.ctaMessage') }}
        </p>
        <NuxtLink
          :to="localePath('/gio-hang')"
          class="inline-flex items-center justify-center px-6 py-3 min-h-[44px]
                 border-2 border-dark text-dark font-condensed text-xs uppercase tracking-[0.15em]
                 hover:bg-dark hover:text-white transition-colors duration-200"
        >
          {{ t('products.ctaButton') }}
        </NuxtLink>
      </div>
    </section>
  </div>
</template>
